...
logfileName = '/home/hadoop/praveen/misshrbt/logs/hive_validation.log'.format(datetime.now())
...
def AddORModifyHivePartition(spark, df):
    ...
    try:
        if row[0] == "AssetId":
            ...
            logging.info("validating Partition for Streaming assetday %s-%s " % (assetid, ftpartitionDate))
            #show partitions gep_data_v2.assetid_BL000470 partition (assetid='BL000470',date_stamp='20210716');
            strQuery = "show partitions gep_data_v2.assetid_%s partition (assetid='%s',date_stamp='%s')" \
                % (assetid, assetid, ftpartitionDate)
...
class hivePartitionCreationStreaming:
    ...
    def StartProcessing(self ):
        ...
        inputFilePath = "/home/hadoop/praveen/misshrbt/inputs/input-stream.csv"
