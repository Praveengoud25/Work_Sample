...
log_file="/home/hadoop/praveen/misshrbt/logs/s3move_$timestamp.log"
error_log="/home/hadoop/praveen/misshrbt/logs/error_$timestamp.log"
...
# This logs a code execution message with the source and destination folders being processed.
  log_code "Moving objects from '$source_file' to '$destination_file'"
  ...
  aws s3 mv "$source_file" "$destination_file" --recursive --sse aws:kms --sse-kms-key-id e18cb522-8757-4623-bd77-8ef0a871e188
